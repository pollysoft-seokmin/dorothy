    ...
    async (entries: UploadEntry[], baseFolderId: string | null) => {
      ...
      setPending((prev) => [
        ...
        ...jobs.map<PendingItem>((j) => ({
          ...
          progress: 0,
          // 변환 필요로 판명되면 아래에서 segments=2 로 승격. 기본 1(업로드만).
          segments: 1,
          step: 0,
        })),
      ])
      ...
      for (const job of jobs) {
        ...
        // 변환을 거치는 파일은 그 몫을 2구간(변환=앞 절반, 업로드=뒤 절반)으로 나눈다.
        ...
        if (job.mediaType === 'video') {
          ...
          if (need) {
            ...
            // 변환 구간(앞 절반) — segments=2 로 승격, step 0. 게이지 0→0.5.
            setItem(job.key, { phase: 'transcoding', progress: 0, segments: 2, step: 0 })
            try {
        ...
        // 업로드 구간 — 변환을 거쳤으면 2구간 중 step 1(뒤 절반), 아니면 단일 구간.
        setItem(job.key, {
          phase: 'uploading',
          progress: 0,
          segments: didTranscode ? 2 : 1,
