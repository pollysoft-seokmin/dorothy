...
export function LanguageToggle({
  ...
  return (
    <Button
      ...
      // 평시에는 배경 투명 + primary(그린) 아이콘이고, hover 시 화이트 10% 둥근
      // 하이라이트로 버튼 영역을 인지시킨다 (다크 배경에서 아이콘만 떠 있으면
      // 클릭 가능성을 알기 어려운 회귀 보완). 아이콘 자체는 primary-bright 로 살짝 밝아짐.
      className="size-10 shrink-0 rounded-full bg-transparent text-primary hover:bg-white/10 hover:text-primary-bright"
    >
      <MaskedIcon name={ICON_BY_LANGUAGE[language]} className="flex size-6 shrink-0" />
    </Button>
  )
}
