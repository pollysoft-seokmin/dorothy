...
Dorothy는 사용자가 로컬 미디어 파일(MP3 오디오·MP4/WebM/MOV 비디오, MPG/MPEG는 클라이언트 트랜스코딩)을 선택하여 브라우저에서 재생하고, 동기화된 가사(LRC)를 카라오케 스타일로 표시하는 웹 애플리케이션입니다. 모든 처리는 클라이언트에서 수행되며, 서버에 파일을 업로드하지 않습니다.
...
| 비디오 (네이티브) | `.mp4`, `.webm`, `.mov` | H.264/AAC MP4 권장. 메타데이터 미파싱(파일명 표시) |
| 비디오 (트랜스코딩) | `.mpg`, `.mpeg` | ffmpeg.wasm으로 H.264/AAC MP4로 클라이언트 변환 후 재생. 첫 사용 시 코어(~25MB) lazy 다운로드 (CDN 캐시) |
| 가사 | `.lrc` | UTF-8 |
...
- **CSR 우선, 인증은 서버**: 미디어/파일 처리는 100% 클라이언트, 인증·개인화 데이터는 서버 저장.
- **서버 업로드 없음**: 미디어/LRC 파일은 절대 서버로 전송되지 않음 (파일명/메타데이터만 이력에 기록).
- **외부 의존성 최소화**: LRC 파서와 ID3 태그 리더 모두 직접 구현 (외부 라이브러리 없음).

### 1.3 인증 / 접근 정책

- **인증 수단**: **Google OAuth 전용**. 이메일/비밀번호 로그인은 지원하지 않으며, 가입 라우트(`/signup`)와 better-auth 의 이메일 가입/로그인 엔드포인트가 모두 비활성(`emailAndPassword` 미설정)이다.
- **접근 정책 (현재)**: **Google 인증을 통과한 사용자는 전원 허용**한다. 도메인 제한·이메일 허용목록·가입 차단이 없어, 임의의 Google 계정으로 로그인하면 신규 user/account 가 자동 생성된다(사실상 공개 등록). — 결정일 2026-06-20.
- **근거**: 운영 단순성과 초기 사용자 확보를 우선. 콘텐츠/기능 자체가 민감하지 않고, 비용을 좌우하는 자원(대역폭·저장)은 Google Drive 사용자 본인 계정에서 나가므로 무제한 가입의 서버 비용 리스크가 낮다.
- **향후 (예정)**: 과금 정책 도입 시 **라이선스/구독 부여 상태에 따라 접근 또는 기능을 제한**할 수 있다. 게이팅 메커니즘·요금제 설계는 [`MONETIZATION.md`](./MONETIZATION.md) 의 "라이선스 기반 접근/기능 게이팅" 절을 참조.
...
| [@ffmpeg/ffmpeg (WASM)](https://github.com/ffmpegwasm/ffmpeg.wasm) | `.mpg`/`.mpeg` → H.264/AAC MP4 클라이언트 트랜스코딩 (`src/lib/transcode.ts`, lazy import + CDN, 단일 스레드 코어) |
...
## 15.5 Authentication & Personalization

### 15.5.1 인증 스택

| 영역 | 선택 |
|------|------|
| 인증 라이브러리 | [Better Auth](https://better-auth.com) |
| DB | Neon Postgres (pooled connection) |
| ORM | Drizzle ORM (`drizzle-orm`, `drizzle-kit`) |
| 로그인 수단 | 이메일+비밀번호 (8자 이상), Google OAuth |
| 세션 | httpOnly 쿠키, 30일 만료, 24h마다 갱신 |
...
### 15.5.3 데이터 모델 (`src/server/db/schema.ts`)

| 테이블 | 비고 |
|--------|------|
| `user`, `session`, `account`, `verification` | Better Auth 표준 |
| `user_preferences` | `user_id` PK, `volume`, `theme`, `updated_at` |
| `playback_history` | `id`, `user_id`, `title`/`artist`/`album`/`file_name`/`duration_seconds`, `last_played_at` (인덱스: `(user_id, last_played_at)`) |

### 15.5.4 동기화 전략

- 로그인 시 `usePreferencesSync` 훅이 server function `getMyPreferences`로 prefs를 한 번 하이드레이트.
- 이후 Zustand store의 `volume` 변경을 500ms 디바운스로 `updateMyPreferences`에 PUT.
- 트랙이 `playing`으로 진입할 때 `usePlaybackHistorySync`가 메타데이터(파일명/제목/아티스트/앨범/길이)만 `appendPlaybackHistory`로 기록 (파일 콘텐츠는 절대 업로드 안 함).
- 비로그인 사용자는 위 훅들이 no-op이며 기존 동작 그대로.

### 15.5.5 환경변수

`.env.example`에 정의된 슬롯:

| 키 | 비고 |
|----|------|
| `DATABASE_URL` | Neon pooled connection string |
| `BETTER_AUTH_SECRET` | `openssl rand -base64 32`로 생성 |
| `BETTER_AUTH_URL` | 절대 URL (로컬: `http://localhost:3000`) |
| `BETTER_AUTH_TRUSTED_ORIGINS` | preview 도메인 등 콤마 구분 (선택) |
| `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` | 둘 다 있을 때만 Google 로그인 활성 |
